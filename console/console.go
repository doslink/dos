...
import (
	...
	"github.com/doslink/dos/internal/jsre"
	"github.com/doslink/dos/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Doslink requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jdos", struct{}{})

	jdosObj, _ := c.jsre.Get("jdos")
	jdosObj.Object().Set("send", bridge.Send)
	jdosObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jdos);"); err != nil {
	...
	flatten := "var dos = web3.dos; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = dos.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jdos.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jdos.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jdos.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jdos.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jdos.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines dos.getBalance(dos.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gdos instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gdos metadata
	fmt.Fprintf(c.printer, "Welcome to the Gdos JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + dos.coinbase);
		console.log("at block: " + dos.blockNumber + " (" + new Date(1000 * dos.getBlock(dos.blockNumber).timestamp) + ")");
