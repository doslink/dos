...
func newTester(t *testing.T, confOverride func(*dos.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Doslink service within
	...
	dosConf := &dos.Config{
		...
		Doserbase: common.HexToAddress(testAddress),
		Dosash: dosash.Config{
			PowMode: dosash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(dosConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return dos.New(ctx, dosConf) }); err != nil {
		t.Fatalf("failed to register Doslink protocol: %v", err)
	}
