...
import (
	...
	"github.com/doslink/dos/signer/core"
	"github.com/doslink/dos/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx dosapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx dosapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(dosapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := dosapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx dosapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
