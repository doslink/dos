...
// Package miner implements Doslink block creation and mining.
package miner
...
func New(dos Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		dos:      dos,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, dos, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(dos.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) SetDoserbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setDoserbase(addr)
}
