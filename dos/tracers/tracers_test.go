...
import (
	...
	"github.com/doslink/dos/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gdos console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = dos.getBlock(dos.getTransaction(tx).blockHash);
  var genesis = dos.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.dos.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  dos.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(dosdb.NewMemDatabase(), test.Genesis.Alloc)
