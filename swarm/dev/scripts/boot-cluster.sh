...
#
# The cluster contains a bootnode, a gdos node and multiple swarm nodes, with
...
# static gdos configuration
GDOS_IP="192.168.33.3"
GDOS_RPC_PORT="8605"
GDOS_RPC_URL="http://${GDOS_IP}:${GDOS_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gdos and
# swarm nodes
start_bootnode() {
...
# start_gdos_node starts a gdos node with --datadir pointing at <base-dir>/gdos
# and a single, unlocked account with password "gdos"
start_gdos_node() {
  local dir="${base_dir}/gdos"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gdos account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GDOS_IP}"
    --rpcport   "${GDOS_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gdos" "${GDOS_IP}" "$(which gdos)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the vdos pair
  local vdos0="vdos${name}0"
  local vdos1="vdos${name}1"
  ip link add name "${vdos0}" type vdos peer name "${vdos1}"

  # add one end to the bridge
  ip link set dev "${vdos0}" master "${BRIDGE_NAME}"
  ip link set dev "${vdos0}" up

  # add the other end to the namespace, rename it dos0 and give it the ip
  ip link set dev "${vdos1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vdos1}" name "dos0"
  ip netns exec "${name}" ip link set dev "dos0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "dos0"
}
...
create_account() {
  ...
  gdos --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
