...
import (
	...
	"github.com/doslink/dos/swarm/storage"
)
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.dos"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.dos"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.dos"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	dosAddr := "swarm.dos"
	dosHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	dosResolve := newTestResolver(dosHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(dosResolve, "")),
			addr:   dosAddr,
			result: dosHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(dosResolve, ""),
				MultiResolverOptionWithResolver(dosResolve, ""),
			),
			addr:   dosAddr,
			result: dosHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(dosResolve, "dos"),
			),
			addr:   dosAddr,
			result: dosHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "dos"),
				MultiResolverOptionWithResolver(doesntResolve, "dos"),
				MultiResolverOptionWithResolver(dosResolve, "dos"),
			),
			addr:   dosAddr,
		...
		},
		{
			...
			addr:   dosAddr,
			result: dosHash,
			err:    errors.New(`DNS name not found: "swarm.dos"`),
		},
		{
