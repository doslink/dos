...
func (n *Node) OpenDatabase(name string, cache, handles int) (dosdb.Database, error) {
	if n.config.DataDir == "" {
		return dosdb.NewMemDatabase(), nil
	}
	return dosdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
