...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (dosdb.Database, error) {
	if ctx.config.DataDir == "" {
		return dosdb.NewMemDatabase(), nil
	}
	db, err := dosdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
