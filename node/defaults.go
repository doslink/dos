...
const (
	...
	DefaultHTTPPort = 8605        // Default TCP port for the HTTP RPC server
	DefaultWSHost   = "localhost" // Default host interface for the websocket RPC server
	DefaultWSPort   = 8606        // Default TCP port for the websocket RPC server
)
...
func DefaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "Doslink")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "Doslink")
		} else {
			return filepath.Join(home, ".doslink")
		}
	}
