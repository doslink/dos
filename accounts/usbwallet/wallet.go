...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Doslink
...
type wallet struct {
	...
	deriveChain    doslink.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Doslink account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain doslink.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
