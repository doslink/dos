...
import (
	...
	"github.com/doslink/dos/accounts/usbwallet/internal/trezor"
...
// or whether the Doslink app was not started on it.
...
// Doslink address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.DoslinkAddress)
	if _, err := w.trezorExchange(&trezor.DoslinkGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.DoslinkTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.DoslinkTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
