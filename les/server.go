...
func NewLesServer(dos *dos.Doslink, config *dos.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(dos.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, dos.EventMux(), dos.Engine(), newPeerSet(), dos.BlockChain(), dos.TxPool(), dos.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(dos.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(dos.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(dos.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(dos.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(dos.ChainDb())
	return srv, nil
}
...
func newCostStats(db dosdb.Database) *requestCostStats {
