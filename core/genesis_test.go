...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		name       string
		fn         func(dosdb.Database) (*params.ChainConfig, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db dosdb.Database) (*params.ChainConfig, common.Hash, error) {
				return SetupGenesisBlock(db, new(Genesis))
			},
			wantErr:    errGenesisNoConfig,
			wantConfig: params.AllDosashProtocolChanges,
		},
		{
		...
		},
		{
			...
			fn: func(db dosdb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, nil, oldcustomg.Config, dosash.NewFullFaker(), vm.Config{})
				defer bc.Stop()

				blocks, _ := GenerateChain(oldcustomg.Config, genesis, dosash.NewFaker(), db, 4, nil)
