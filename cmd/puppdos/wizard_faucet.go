...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{port: 30605, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	fmt.Printf("How many Dosers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.dosstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.dosstats = w.readString() + ":" + w.conf.dosstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.dosstats)
		infos.node.dosstats = w.readDefaultString(infos.node.dosstats) + ":" + w.conf.dosstats
	}
