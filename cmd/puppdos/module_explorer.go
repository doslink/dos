...
FROM puppdos/explorer:latest

ADD dosstats.json /dosstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../dos-net-intelligence-api && pm2 start /dosstats.json)' >  explorer.sh && \
	echo '(cd ../doserchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerDosstats is the configuration file for the dosstats javascript client.
var explorerDosstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.doslink
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Dosstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	dosstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerDosstats)).Execute(dosstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.dosstats[:strings.Index(config.dosstats, ":")],
		"Secret": config.dosstats[strings.Index(config.dosstats, ":")+1 : strings.Index(config.dosstats, "@")],
		"Host":   config.dosstats[strings.Index(config.dosstats, "@")+1:],
	})
	files[filepath.Join(workdir, "dosstats.json")] = dosstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.doslink"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		dosstats: infos.envvars["STATS"],
	}
	return stats, nil
}
