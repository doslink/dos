...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for dosstats availability")
	if infos, err := checkDosstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["dosstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["dosstats"] = infos.Report()
		dosstats = infos.config
	}
