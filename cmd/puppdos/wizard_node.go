...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.dosstats == "" {
		log.Error("No dosstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{port: 30605, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{port: 30605, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if w.conf.Genesis.Config.Dosash != nil && !boot {
		fmt.Println()
		if infos.dosashdir == "" {
			fmt.Printf("Where should the dosash mining DAGs be stored on the remote machine?\n")
			infos.dosashdir = w.readString()
		} else {
			fmt.Printf("Where should the dosash mining DAGs be stored on the remote machine? (default = %s)\n", infos.dosashdir)
			infos.dosashdir = w.readDefaultString(infos.dosashdir)
		}
	}
	...
	if infos.dosstats == "" {
		...
		infos.dosstats = w.readString() + ":" + w.conf.dosstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.dosstats)
		infos.dosstats = w.readDefaultString(infos.dosstats) + ":" + w.conf.dosstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Dosash != nil {
			// Dosash based miners only need an doserbase to mine against
			fmt.Println()
			if infos.doserbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.doserbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.doserbase)
				infos.doserbase = w.readDefaultAddress(common.HexToAddress(infos.doserbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Doslink node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
