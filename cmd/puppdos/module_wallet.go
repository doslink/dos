...
FROM puppdos/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gdos --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gdos --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --dosstats \'{{.Dosstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppdosNetworkID/{{.NetworkID}}/g' dist/js/doserwallet-master.js && \
	sed -i 's/PuppdosNetwork/{{.Network}}/g'     dist/js/doserwallet-master.js && \
	sed -i 's/PuppdosDenom/{{.Denom}}/g'         dist/js/doserwallet-master.js && \
	sed -i 's/PuppdosHost/{{.Host}}/g'           dist/js/doserwallet-master.js && \
	sed -i 's/PuppdosRPCPort/{{.RPCPort}}/g'     dist/js/doserwallet-master.js
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:8605"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.doslink
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["8605/tcp"]
