...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"dosstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "dosstats":
				if infos, err := checkDosstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have dosstats running, ask whether to make the secret public or not
	if w.conf.dosstats != "" {
		fmt.Println()
		fmt.Println("Include dosstats secret on dashboard (y/n)? (default = yes)")
