...
func TestAccountListEmpty(t *testing.T) {
	gdos := runGdos(t, "account", "list")
	gdos.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gdos := runGdos(t, "account", "list", "--datadir", datadir)
	defer gdos.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gdos := runGdos(t, "account", "new", "--lightkdf")
	defer gdos.ExpectExit()
	gdos.Expect(`
...
`)
	gdos.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gdos := runGdos(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gdos.ExpectExit()
	gdos.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gdos.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gdos.StderrText(), m) {
...
// https://github.com/doslink/dos/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gdos.SetTemplateFunc("keypath", func(file string) string {
