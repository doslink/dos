...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb  = dosdb.NewMemDatabase()
		lightdb = dosdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, dosash.NewFullFaker(), vm.Config{})
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, dosash.NewFaker(), fulldb, 4, testChainGen)
