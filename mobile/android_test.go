...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gdos.LightScryptN, Gdos.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.doslink", new NodeConfig());
			node.start();
			...
			DoslinkClient ec = node.getDoslinkClient();
	...
	// signatures too. Regression test for dos issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gdos-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/doslink/gdostest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gdos and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.doslink", "github.com/doslink/dos/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gdos.aar"), "gdos.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "doslink", "gdostest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
