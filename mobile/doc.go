...
// Package gdos contains the simplified mobile APIs to dos.
//
// The scope of this package is *not* to allow writing a custom Doslink client
// with pieces plucked from dos, rather to allow writing native dapps on
...
// from the dos repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original dos version.
